<?php $portfolio_list = carbon_get_the_post_meta('gallery_list'); ?>
<div
    class="tile <?php if ( $portfolio_list ) : ?> tile-is-movable <?php endif; ?> tile-hidden w-full sm:w-1/2 px-3 mb-6">
    <div class="relative aspect-ratio-square overflow-hidden">
        <div
        ...
        >
            <div class="absolute inset-0 w-full h-full p-8 flex flex-col justify-end">
                ...
                <div
                    class="tile-action-icon tile-action-icon-gallery top-0 left-0 mt-6 ml-6 md:mt-0 md:ml-auto absolute md:top-auto right-0 mx-auto md:opacity-0 transition-opacity duration-500 ease-in-out rounded-full md:bottom-1/2 md:transform md:translate-y-1/2 bg-center bg-no-repeat"
            ...
            >
                <div class="p-8 overflow-auto tile-content-inner">
                    <?php if ( carbon_get_the_post_meta('gallery_list_heading') ) : ?>
                    <div class="text-white uppercase font-bold mb-5">
                        <?php echo carbon_get_the_post_meta('gallery_list_heading'); ?></div>
                    <?php endif; ?>
                    <ul class="tile-list text-sm text-white">
                        <?php foreach ( $portfolio_list as $list_item ) : ?>
                        <li class="relative pl-8"><?php echo $list_item['gallery_list_item']; ?></li>
                        <?php endforeach; ?>
                    </ul>
                </div>
            </div>
